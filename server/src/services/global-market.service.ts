...
interface MarketIntelligence {
  timestamp: string;
  giftNifty: { value: number; change: number; changePercent: number } | null;
...
let _latestIntelligence: MarketIntelligence | null = null;
...
export class GlobalMarketService {
  ...
  async runDailyIntelligenceScan(): Promise<MarketIntelligence> {
    ...
    let aiSummary = '';
    let sentiment: MarketIntelligence['sentiment'] = 'NEUTRAL';
    let keyEvents: string[] = [];
    let stockFlows: MarketIntelligence['stockFlows'] = { mfTopBuys: [], mfTopSells: [], fiiTopBuys: [], fiiTopSells: [] };
...
Respond in JSON with this schema:
{
  ...
  "stockCategories": {
    ...
    "smallCap": "brief outlook"
      ...
      console.error('[GlobalMarket] AI analysis failed:', (err as Error).message);
    ...
    try {
      ...
      console.error('[GlobalMarket] Failed to store intelligence:', (err as Error).message);
    }
  ...
  getIntelligenceContextForBots(): string {
    ...
    const usIndices = intel.globalIndices.filter(i =>
      ['S&P 500', 'NASDAQ', 'DOW JONES'].some(n => i.name.toUpperCase().includes(n.toUpperCase()))
    );
  ...
  private async fetchGiftNifty(): Promise<{ value: number; change: number; changePercent: number } | null> {
    try {
  ...
  private async fetchYahooChart(ticker: string): Promise<{ value: number; change: number; changePercent: number } | null> {
    try {
  ...
  private async fetchGlobalIndices(): Promise<GlobalIndex[]> {
  ...
  private async fetchSectorPerformance(): Promise<{ sector: string; changePercent: number }[]> {
    const sectorIndices: [string, string][] = [
      ['NIFTY IT.NS', 'IT'],
    ...
    const sectors: { sector: string; changePercent: number }[] = [];

    // Try Yahoo Finance sector tickers
    const yahooTickers: [string, string][] = [
  ...
  private generateRuleBasedSummary(
    giftNifty: { value: number; change: number; changePercent: number } | null,
    globalIndices: GlobalIndex[],
    fiiDii: { fiiNet: number; diiNet: number },
  ...
  private computeRuleBasedSentiment(
    giftNifty: { changePercent: number } | null,
    globalIndices: GlobalIndex[],
    fiiDii: { fiiNet: number },
  ): MarketIntelligence['sentiment'] {
    let score = 0;

    if (giftNifty) {
      if (giftNifty.changePercent > 0.5) score += 2;
      else if (giftNifty.changePercent > 0.1) score += 1;
      else if (giftNifty.changePercent < -0.5) score -= 2;
      else if (giftNifty.changePercent < -0.1) score -= 1;
    }
    ...
    if (us.length > 0) {
      const avgUs = us.reduce((s, i) => s + i.changePercent, 0) / us.length;
      if (avgUs > 0.5) score += 2;
      else if (avgUs > 0.1) score += 1;
      else if (avgUs < -0.5) score -= 2;
      else if (avgUs < -0.1) score -= 1;
    }

    if (fiiDii.fiiNet > 500) score += 2;
    else if (fiiDii.fiiNet > 0) score += 1;
    else if (fiiDii.fiiNet < -500) score -= 2;
    else if (fiiDii.fiiNet < 0) score -= 1;

    if (score >= 4) return 'VERY_BULLISH';
    if (score >= 2) return 'BULLISH';
    if (score <= -4) return 'VERY_BEARISH';
  ...
  private async storeIntelligence(intel: MarketIntelligence): Promise<void> {
    ...
    try {
      ...
      console.log(`[GlobalMarket] ${date} — Sentiment: ${intel.sentiment} (DB store skipped: ${(err as Error).message})`);
    }
  }
}
