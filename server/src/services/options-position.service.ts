...
export class OptionsPositionService {
  ...
  async getOptionsPortfolioGreeks(userId: string, spotPrice?: number): Promise<PortfolioGreeks> {
    ...
    const optionPositions = positions.filter(p =>
      p.strategyTag?.includes('OPT:') || p.symbol.match(/(CE|PE)$/i)
    );

    if (optionPositions.length === 0) return this.emptyGreeks();
    ...
    for (const pos of optionPositions) {
      ...
      const optType = (parts[1] ?? (pos.symbol.endsWith('CE') ? 'CE' : 'PE')) as 'CE' | 'PE';
