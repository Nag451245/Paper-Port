...
import type { BacktestMetrics, BacktestTrade } from './backtest-engine.service.js';
...
export class WalkForwardOptimizer {
  ...
  run(config: WalkForwardConfig): WalkForwardResult {
    ...
    log.info(
      { symbol, windows: windowCount, combinations: paramCombinations.length, bars: bars.length },
    ...
    for (let w = 0; w < windowCount; w++) {
      ...
      log.debug(
        { window: w, isBars: inSampleBars.length, oosBars: outOfSampleBars.length },
        'Processing window',
      );
      ...
      for (const params of paramCombinations) {
        ...
        try {
          ...
          log.debug({ params, error: (e as Error).message }, 'Parameter combination failed');
        }
      }
    ...
    log.info(
      { overfitRatio, robustnessScore, isOOSProfitable: aggregateOOSReturn > 0, windows: windows.length },
  ...
  private cloneStrategyWithParams(strategy: Strategy, params: Record<string, number>): Strategy {
    const Constructor = strategy.constructor as new (params: Record<string, unknown>) => Strategy;
    return new Constructor(params);
  }

  private computeAggregateMetrics(trades: BacktestTrade[], initialCapital: number): BacktestMetrics {
