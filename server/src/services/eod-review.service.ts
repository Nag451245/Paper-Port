...
export class EODReviewService {
    ...
    try {
      ...
      for (const user of users) {
        try {
          ...
          console.error(`[EODReview] Error reviewing user ${user.id}:`, (err as Error).message);
        }
      }
    } finally {
      this.running = false;
  ...
  private async reviewUserDay(userId: string): Promise<void> {
    ...
    // Trade reviews
    const tradeReviews: TradeReview[] = trades.map(t => ({
      tradeId: t.id,
      symbol: t.symbol,
      side: t.side,
    ...
    const falsePositives: FalsePositive[] = signals
      .filter(s => {
    ...
    // Market context
    let marketContext: { vix: number; niftyChange: number } = { vix: 15, niftyChange: 0 };
    try {
      const [vixData, indices] = await Promise.all([
        this.marketData.getVIX().catch(() => ({ value: 15 })),
        this.marketData.getIndices().catch(() => []),
      ]);
      const nifty = (indices as any[]).find((i: any) => i.name?.includes('NIFTY'));
      marketContext = { vix: (vixData as any).value, niftyChange: nifty?.changePercent ?? 0 };
    } catch { /* use defaults */ }
    ...
    try {
      aiReview = await chatCompletionJSON<typeof aiReview>({
        messages: [
          {
            role: 'system',
...
${falsePositives.map(fp => `  ${fp.direction} ${fp.symbol} (${(fp.confidence * 100).toFixed(0)}% conf) → ${fp.actualOutcome}`).join('\n')}`,
    ...
    try {
      ...
      console.error(`[EODReview] Learning store write failed:`, (err as Error).message);
    }
  ...
  private async triggerDeepReview(userId: string, lossDays: number): Promise<void> {
    ...
    try {
      ...
      console.error(`[EODReview] Deep review AI failed:`, (err as Error).message);
    }
  }
  ...
  async getReports(userId: string, limit = 30) {
