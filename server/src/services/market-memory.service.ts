...
function computeNiftyBand(niftyLevel: number): string {
  const base = Math.floor(niftyLevel / 500) * 500;
  return `${base}-${base + 500}`;
}

function getAdjacentBands(niftyLevel: number): string[] {
  const base = Math.floor(niftyLevel / 500) * 500;
  return [
    `${base - 500}-${base}`,
    `${base}-${base + 500}`,
...
function parseFingerprint(fp: string): number[] {
  try {
...
export class MarketMemoryService {
  ...
  async recordMemory(input: MemoryRecordInput): Promise<string> {
    ...
    const record = await this.prisma.marketMemory.create({
      data: {
        ...
        marketSnapshot: input.marketSnapshot ? (input.marketSnapshot as any) : undefined,
      },
    });
  ...
  async getWinRateForConditions(
    userId: string,
    regime: string,
    niftyBand: string,
    strategy?: string,
  ): Promise<{ winRate: number; totalCases: number }> {
    ...
    if (strategy) where.signalStrategy = strategy;
    ...
    if (total === 0) return { winRate: 0, totalCases: 0 };
  ...
  async getSupportResistanceLevels(
    ...
    const levelMap = new Map<
      number,
      { wins: number; losses: number; lastTested: Date; directions: string[] }
    >();
    ...
    for (const m of memories) {
      ...
      if (m.outcome === 'WIN') entry.wins++;
      else if (m.outcome === 'LOSS') entry.losses++;

      if (m.timestamp > entry.lastTested) entry.lastTested = m.timestamp;
  ...
  private generateCautionNotes(
    resolved: Array<{ record: { outcome: string | null; signalStrategy: string; vixLevel: number } }>,
    conditions: MarketConditions,
  ): string[] {
    const notes: string[] = [];

    const strategyGroups = new Map<string, { wins: number; total: number }>();
    for (const s of resolved) {
      const key = s.record.signalStrategy;
      const entry = strategyGroups.get(key) ?? { wins: 0, total: 0 };
      entry.total++;
      if (s.record.outcome === 'WIN') entry.wins++;
    ...
    const highVixLosses = resolved.filter(
      (s) => s.record.vixLevel >= 18 && s.record.outcome === 'LOSS',
    );
    const totalLosses = resolved.filter((s) => s.record.outcome === 'LOSS');
    if (totalLosses.length >= 3 && highVixLosses.length >= 3) {
      notes.push(
        `VIX was 18+ in ${highVixLosses.length}/${totalLosses.length} losing cases`,
      );
    }
  ...
  private computePriceBandSize(prices: number[]): number {
