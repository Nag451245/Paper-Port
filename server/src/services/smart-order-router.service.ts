...
export class SmartOrderRouterService {
  private fillStats = new Map<string, ExchangeFillStats>();
  ...
  route(order: {
    ...
    if (order.exchange) {
      const explicit = order.exchange.toUpperCase() as 'NSE' | 'BSE';
  ...
  private getFillQualityScore(exchange: string): number {
    const stats = this.fillStats.get(exchange);
    if (!stats || stats.recentSlippages.length === 0) return 0;
    return stats.recentSlippages.reduce((a, b) => a + b, 0) / stats.recentSlippages.length;
  }

  private getExpectedSpread(symbol: string, exchange: string): number {
  ...
  private computeConfidence(spreadDiffBps: number, fillQuality: number): number {
    let confidence = 0.6;
    if (spreadDiffBps > 5) confidence += 0.15;
    else if (spreadDiffBps > 2) confidence += 0.1;

    if (fillQuality > 0 && fillQuality < 2) confidence += 0.1;
    else if (fillQuality >= 2 && fillQuality < 5) confidence += 0.05;
