...
export class FeaturePipelineService {
  ...
  extractFeatures(
    ...
    memoryRecall?: { winRate: number; avgPnl: number; sampleCount: number; confidence: number },
  ): FeatureVector {
    const n = bars.length;
    if (n === 0) {
    ...
    const safeReturn = (periodsBack: number): number => {
      if (n <= periodsBack) return 0;
  ...
  normalizeFeatures(
    ...
    if (runningMean && runningStd && runningMean.length === n && runningStd.length === n) {
      return features.map((v, i) =>
        runningStd[i] !== 0 ? (v - runningMean[i]) / runningStd[i] : 0,
      );
    }
  ...
  private computeSMA(values: number[], period: number): number[] {
  ...
  private computeRSI(closes: number[], period: number): number[] {
    ...
    if (deltas.length < period) return [];
  ...
  private computeATR(bars: Bar[], period: number): number[] {
  ...
  private computeMACD(
    ...
    if (closes.length === 0) return { macd: [], signal: [], histogram: [] };
  ...
  private computeBollingerBands(
    ...
    if (middle.length === 0) return { upper: [], middle: [], lower: [], width: [] };
  ...
  private computeVWAP(bars: Bar[]): number[] {
