...
function computeATRSeries(bars: Bar[], period: number): number[] {
...
function computeADX(bars: Bar[], period: number = 14): number {
...
function computeBBWidth(closes: number[], period: number = 20): { width: number; series: number[] } {
  const widths: number[] = [];
...
function computeTrendStrength(bars: Bar[], period: number = 20): number {
...
export class RegimeDetectorService {
  ...
  async detectFromHistory(symbol: string, exchange: string, days: number): Promise<RegimeResult> {
    ...
    const bars: Bar[] = historicalBars.map((hb: any) => ({
  ...
  async detectHybrid(bars: Bar[], vix?: number): Promise<{
    regime: MarketRegime;
    confidence: number;
    indicators: RegimeIndicators;
    ...
    let mlRegime: string | undefined;
    let source: 'rule_based' | 'hybrid' | 'ml' = 'rule_based';
    ...
    try {
      if (await isMLServiceAvailable() && bars.length >= 30) {
        ...
        const mlToRegime: Record<string, MarketRegime> = {
        ...
        const mlWeight = 0.4;
        const mlConfidence = (mlResult as any).confidence ?? 0.6;
  ...
  async detectAndStore(bars: Bar[], vix?: number, symbol?: string, exchange?: string): Promise<{
    regime: MarketRegime;
    confidence: number;
    source: string;
  }> {
