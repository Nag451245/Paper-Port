...
import type { Strategy, Bar, Signal, Fill, StrategyContext, MarketRegime } from './strategy-sdk.js';
...
export class BacktestEngine {
  run(config: BacktestConfig): BacktestResult {
    ...
    log.info(
      { symbol, totalTrades: metrics.totalTrades, totalReturn: metrics.totalReturn, sharpe: metrics.sharpeRatio },
      'Backtest complete',
    );
  ...
  private computeCommission(price: number, qty: number, commissionPct: number): number {
  ...
  private checkStopsAndTargets(
    ...
    for (let j = openPositions.length - 1; j >= 0; j--) {
      ...
      if (exitPrice !== null) {
        const slippedExit = this.applySlippage(
          exitPrice,
  ...
  private computeEquity(cash: number, openPositions: OpenPosition[], bar: Bar): number {
