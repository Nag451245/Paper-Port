...
export class POVExecutorService {
  ...
  private activeExecutions = new Map<string, { config: POVConfig; slices: POVSlice[]; cancelled: boolean }>();
  ...
  async execute(config: POVConfig): Promise<{
    executionId: string;
    slices: POVSlice[];
    avgFillPrice: number;
    totalFilled: number;
    ...
    const execution = { config, slices: [] as POVSlice[], cancelled: false };
