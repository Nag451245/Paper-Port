...
export class MonteCarloSimulator {
  run(config: MonteCarloConfig): MonteCarloResult {
    ...
    const finalReturns: number[] = new Array(iterations);
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      const { finalEquity, maxDrawdownPct, hitRuin } = this.simulateEquityCurve(
        shuffled, initialCapital, ruinLevel,
      );
  ...
  private simulateEquityCurve(
    ...
    initialCapital: number,
    ruinLevel: number,
  ): { finalEquity: number; maxDrawdownPct: number; hitRuin: boolean } {
