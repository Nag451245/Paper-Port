...
interface BookState {
  ...
  cumulativeVwapNumerator: number;
}
...
export class OrderBookService {
  ...
  getSpread(symbol: string): number {
    const state = this.books.get(symbol);
    if (!state || state.bids.length === 0 || state.asks.length === 0) return 0;
  ...
  private getOrCreate(symbol: string): BookState {
  ...
  private computeImbalance(state: BookState): number {
  ...
  private trackSpread(state: BookState): void {
  ...
  private calibrateLambda(state: BookState): number {
    if (state.recentSpreads.length < 5) return DEFAULT_LAMBDA;
    ...
    if (midPrice <= 0) return DEFAULT_LAMBDA;
