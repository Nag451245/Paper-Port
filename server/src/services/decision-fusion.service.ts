...
export interface FusionDecision {
  ...
  reasoning: string[];
  signalSources: {
    rust: { score: number; weight: number };
    ml: { score: number; weight: number };
    memory: { score: number; weight: number; sampleCount: number };
    regime: { current: string; alignment: number };
  };
  memoryContext: {
    similarCases: number;
    historicalWinRate: number;
...
export class DecisionFusionService {
  private minExecuteScore: number;
  private minWatchScore: number;
  ...
  async decide(
    ...
    let action: FusionDecision['action'];
    ...
    let confidence: FusionDecision['confidence'];
    if (finalScore > 0.75) {
      confidence = 'HIGH';
    ...
    const reasoning = this.buildReasoning(
      rustSignal, mlScore, memoryRecall, regime,
      strategyHealth, finalScore, action, regimeAlignment,
      rustWeight, mlWeight, memoryWeight, regimeWeight,
    );
  ...
  async getAdaptiveThresholds(userId: string): Promise<{ minExecute: number; minWatch: number }> {
  ...
  async updateThresholds(userId: string, adjust: number): Promise<void> {
    const redis = getRedis();
    if (!redis) {
    ...
    try {
      ...
      await redis.set(
        `cg:fusion:thresholds:${userId}`,
        JSON.stringify(updated),
        'EX',
        THRESHOLD_TTL_SECONDS,
      );
  ...
  private buildReasoning(
    ...
    lines.push(
      `${rustSignal.symbol} ${rustSignal.direction} signal from Rust engine ` +
    ...
    if (mlScore.available) {
      lines.push(
        `ML model predicts ${(mlScore.winProbability * 100).toFixed(0)}% win probability ` +
    ...
    if (memoryRecall.similarCases > 0) {
      lines.push(
        `Market memory found ${memoryRecall.similarCases} similar setups ` +
    ...
    lines.push(
      `Market regime: ${regime.current} (confidence: ${(regime.confidence * 100).toFixed(0)}%, ` +
      `source: ${regime.source}). ` +
      `Regime alignment with signal direction: ${(regimeAlignment * 100).toFixed(0)}%.`
    );

    lines.push(
      `Weighted fusion: rust=${(rustWeight * 100).toFixed(0)}%, ` +
    ...
    if (strategyHealth.isDecaying || strategyHealth.consecutiveLosses >= 3) {
      lines.push(
        `Strategy health: win rate ${(strategyHealth.recentWinRate * 100).toFixed(0)}%, ` +
    ...
    lines.push(
      `Final score: ${(finalScore * 100).toFixed(1)}% → Action: ${action}.`
    );
