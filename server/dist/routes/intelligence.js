...
import { engineMonteCarlo, enginePortfolioOptimize, engineOptionsStrategy, engineCorrelation, engineFeatureStore, isEngineAvailable, } from '../lib/rust-engine.js';
const symbolParam = z.string().min(1).max(30).regex(/^[A-Z0-9&_-]+$/i, 'Invalid symbol');
function parseSymbol(params) {
    const result = symbolParam.safeParse(params?.symbol);
...
export async function intelligenceRoutes(app) {
    ...
    app.post('/engine/monte-carlo', async (req, reply) => {
        if (!isEngineAvailable())
            return reply.code(503).send(ENGINE_UNAVAILABLE);
        try {
            const body = req.body;
            if (!body?.returns || !Array.isArray(body.returns))
                return reply.code(400).send({ error: 'returns array required' });
    ...
    app.post('/engine/portfolio-optimize', async (req, reply) => {
        ...
        try {
            ...
            if (!body?.assets || !Array.isArray(body.assets))
                return reply.code(400).send({ error: 'assets array required' });
    ...
    app.post('/engine/options-strategy', async (req, reply) => {
        ...
        try {
            ...
            if (!body?.legs || !Array.isArray(body.legs))
                return reply.code(400).send({ error: 'legs array required' });
            if (!body?.spot)
                return reply.code(400).send({ error: 'spot price required' });
    ...
    app.post('/engine/correlation', async (req, reply) => {
        ...
        try {
            ...
            if (!body?.pairs || !Array.isArray(body.pairs))
                return reply.code(400).send({ error: 'pairs array required' });
    ...
    app.post('/engine/features', async (req, reply) => {
        ...
        try {
            const body = req.body;
            if (!body?.command)
                return reply.code(400).send({ error: 'command required (extract_features, detect_regime, detect_anomalies)' });
