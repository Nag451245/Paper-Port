...
export async function authRoutes(app) {
    ...
    const handleBreezeCallback = async (request, reply) => {
        ...
        const token = params.api_session ??
        ...
        if (state) {
            try {
                const payload = app.jwt.verify(state);
                if (payload?.sub && payload.type === 'breeze_session_state') {
                    await authService.saveSessionToken(payload.sub, token);
                    const safeOrigin = escapeHtml(origin);
                    return reply.type('text/html').send(`<html><body><script>window.opener&&window.opener.postMessage({type:"breeze_session_saved"},"${safeOrigin}");window.close();</script><h3>Session saved! You can close this tab.</h3></body></html>`);
                }
            }
            catch {
