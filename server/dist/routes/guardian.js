...
export default async function guardianRoutes(app) {
    ...
    app.get('/state', async (request, reply) => {
        ...
        try {
            ...
            return reply.code(500).send({ error: err.message || 'Failed to load state' });
        }
    });
    app.get('/memories', async (request, reply) => {
        ...
        try {
            ...
            return reply.code(500).send({ error: err.message || 'Failed to load memories' });
        }
    });
    ...
    app.get('/awareness', async (request, reply) => {
        ...
        try {
            ...
            return reply.code(500).send({ error: err.message || 'Failed to load awareness' });
        }
    });
}
