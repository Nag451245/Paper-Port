...
import { OptionsService, calculateStrategyGreeks, calculatePayoffCurve } from '../services/options.service.js';
...
export async function optionsRoutes(app) {
    ...
    app.post('/payoff', async (request, reply) => {
        ...
        const result = optionsService.computePayoff(parsed.data.legs, parsed.data.spotPrice);
        return result;
    });
    app.post('/scenario', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        return optionsService.scenarioSimulation(parsed.data.legs, parsed.data.spotPrice, parsed.data.scenarios);
    });
    app.post('/max-pain', async (request, reply) => {
        ...
        const callOI = {};
        const putOI = {};
        for (const [k, v] of Object.entries(parsed.data.callOI))
            callOI[Number(k)] = v;
        for (const [k, v] of Object.entries(parsed.data.putOI))
            putOI[Number(k)] = v;
    ...
    app.post('/payoff-engine', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        if (isEngineAvailable())
            try {
                ...
                return {
                    source: 'rust',
                    payoffCurve: (result.payoff_diagram ?? []).map((p) => ({ spot: p.price, pnl: Math.round(p.pnl * 100) / 100 })),
        ...
        const range = [spotPrice * 0.8, spotPrice * 1.2];
    ...
    app.post('/optimize', { preHandler: [authenticate] }, async (request, reply) => {
        ...
        if (spot <= 0)
            return reply.send({ strategies: [], message: 'Could not determine spot price' });
        const strikes = chain.strikes;
        const atmStrike = strikes.reduce((best, s) => Math.abs(s.strike - spot) < Math.abs(best.strike - spot) ? s : best, strikes[0]);
        const atmIdx = strikes.findIndex((s) => s.strike === atmStrike.strike);
        const stepSize = strikes.length > 1 ? Math.abs(strikes[1].strike - strikes[0].strike) : 50;
        const getStrike = (offset) => {
        ...
        const evaluated = filtered.map(strategy => {
            ...
            const netPremium = strategy.legs.reduce((s, l) => s + (l.action === 'SELL' ? l.premium * l.qty : -l.premium * l.qty), 0);
            ...
            const avgIV = strategy.legs.reduce((s, l) => {
                const st = strikes.find((ss) => ss.strike === l.strike);
            ...
            return {
                ...
                payoffPreview: payoffCurve.filter((_, i) => i % 3 === 0).map(p => ({ spot: p.spotPrice, pnl: Math.round(p.pnl) })),
