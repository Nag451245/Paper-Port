...
export class RiskService {
    ...
    async enforceTargetRisk(userId, orderValue, symbol, side) {
        const violations = [];
        const warnings = [];
     ...
     */
    async getRegimeRiskOverrides(userId) {
        try {
            ...
            return {
                ...
                maxStopLossPctPerPosition: Math.max(0.1, DEFAULT_CONFIG.maxStopLossPctPerPosition * (1 - limits.stopLossTighten)),
    ...
    async preTradeCheck(userId, symbol, side, qty, price, config) {
        ...
        // Rule 6: Correlation-aware position limits
        {
            ...
            const sameSectorCount = existingPositions.filter(p => (SECTOR_MAP[p.symbol] ?? 'Other') === newSector).length;
            if (sameSectorCount >= cfg.maxCorrelatedPositions) {
                violations.push(`Already ${sameSectorCount} positions in ${newSector} sector (max ${cfg.maxCorrelatedPositions} correlated). ` +
                    `Adding ${symbol} increases concentration risk.`);
            }
            if (sameSectorCount >= cfg.maxCorrelatedPositions - 1) {
                warnings.push(`${sameSectorCount}/${cfg.maxCorrelatedPositions} correlated positions in ${newSector} sector`);
            }
        }
                    ...
                    for (const pos of existingPositions) {
                        ...
                        if (existingReturns) {
                            ...
                            if (corr > cfg.maxCorrelationBetweenPositions) {
                                violations.push(`${symbol} is too correlated with open position ${pos.symbol} (r=${corr.toFixed(2)}, max ${cfg.maxCorrelationBetweenPositions})`);
                            }
        ...
        // Rule 11: Simultaneous risk budget — total open risk must stay under limit
        {
            ...
            const currentRisk = allOpen.reduce((s, p) => s + Number(p.avgEntryPrice) * p.qty * (cfg.maxStopLossPctPerPosition / 100), 0);
        ...
        try {
            ...
            const currentPositionQty = positionsForLimitCheck
                .reduce((sum, p) => sum + Math.abs(p.qty), 0);
    ...
    async getDailyRiskSummary(userId) {
        const portfolios = await this.prisma.portfolio.findMany({
            where: { userId },
            select: { id: true, initialCapital: true, currentNav: true },
        });
    ...
    async getPortfolioVaR(userId, confidenceLevel = 0.95, holdingDays = 1) {
        const emptyResult = { parametricVaR: 0, historicalVaR: 0, var95: 0, var99: 0, expectedShortfall: 0, portfolioValue: 0, positions: [] };
        ...
        if (openPositions.length === 0)
            return { ...emptyResult, portfolioValue: nav };
        const thirtyDaysAgo = new Date();
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
        const recentTrades = await this.prisma.trade.findMany({
    ...
    async getSectorConcentration(userId) {
        ...
        for (const pos of positions) {
            ...
            const value = Number(pos.avgEntryPrice) * pos.qty;
            if (!sectorAgg[sector])
                sectorAgg[sector] = { value: 0, symbols: [] };
            sectorAgg[sector].value += value;
            if (!sectorAgg[sector].symbols.includes(pos.symbol))
                sectorAgg[sector].symbols.push(pos.symbol);
        }
     ...
     */
    computePositionSize(capital, entryPrice, stopLossPrice, config) {
     ...
     */
    async getSizeMultiplier(userId) {
        let multiplier = 1.0;
        // Check weekly loss
        const portfolios = await this.prisma.portfolio.findMany({
            where: { userId },
    ...
    async getComprehensiveRisk(userId) {
     ...
     */
    async forceCloseOnDailyLossLimit(userId, closePositionFn) {
        const portfolios = await this.prisma.portfolio.findMany({
            where: { userId },
            select: { id: true, initialCapital: true },
        });
        if (portfolios.length === 0)
            return { triggered: false, closedCount: 0, dayLossPct: 0 };
        const capital = Number(portfolios[0].initialCapital);
        const todayStart = new Date();
        todayStart.setHours(0, 0, 0, 0);
        const todayTrades = await this.prisma.trade.findMany({
    ...
    async checkConsecutiveLossPause(userId) {
        ...
        // Clear expired pause
        if (existingPause)
            this.pauseUntilMap.delete(userId);
        const portfolios = await this.prisma.portfolio.findMany({
            where: { userId },
            select: { id: true },
        });
        if (portfolios.length === 0)
            return { paused: false, pauseUntil: null, consecutiveLosses: 0 };
        const todayStart = new Date();
        todayStart.setHours(0, 0, 0, 0);
        const recentTrades = await this.prisma.trade.findMany({
            where: { portfolioId: portfolios[0].id, exitTime: { gte: todayStart } },
