...
export class AIAgentService {
    ...
    async analyzeOptionsOpportunity(userId, symbol) {
        try {
            ...
            const ivValues = strikes.map((s) => s.callIV || s.putIV || 0).filter((v) => v > 0);
...
Top 5 Call OI: ${strikeValues.slice(0, 5).map((s) => `${s}:${callOI[s]}`).join(', ')}
Top 5 Put OI: ${strikeValues.slice(0, 5).map((s) => `${s}:${putOI[s]}`).join(', ')}
  ...
  "legs": [{"type":"CE|PE","strike":number,"action":"BUY|SELL","qty":1}]
    ...
    async updateConfig(userId, data) {
        ...
        if (data.strategies !== undefined)
            dbData.strategies = JSON.stringify(data.strategies);
        if (data.capitalPreservationOverrides !== undefined)
            dbData.capitalPreservationOverrides = JSON.stringify(data.capitalPreservationOverrides);
    ...
    async listSignals(userId, params = {}) {
    ...
    async getSignal(signalId, userId) {
    ...
    async executeSignal(signalId, userId) {
        ...
        if (!portfolio)
            throw new AIAgentError('No portfolio found', 404);
        ...
        if (direction === 'BUY') {
            ...
            if (longPosition) {
                ...
                if (exitPrice <= 0)
                    throw new AIAgentError(`Cannot sell ${signal.symbol}: no price available`, 400);
    ...
    async rejectSignal(signalId, userId) {
        const signal = await this.getSignal(signalId, userId);
        if (signal.status !== 'PENDING') {
            throw new AIAgentError('Signal is not in pending status', 400);
        }
    ...
    async getPreMarketBriefing(userId) {
        ...
        if (this.cachedBriefing) {
            ...
            if (ageMs < maxAge)
                return this.cachedBriefing.data;
        }
        // Prevent parallel regeneration
        if (this.briefingInProgress && this.cachedBriefing)
            return this.cachedBriefing.data;
        return this.regenerateBriefing();
    }
    async regenerateBriefing() {
        if (this.briefingInProgress)
        ...
        try {
            ...
            try {
                ...
                const parts = [];
                if (vixData?.value)
                    parts.push(`India VIX: ${vixData.value} (${vixData.change >= 0 ? '+' : ''}${vixData.changePercent.toFixed(1)}%)`);
                if (niftyQuote?.ltp)
                    parts.push(`NIFTY 50: ${niftyQuote.ltp} (${niftyQuote.changePercent >= 0 ? '+' : ''}${niftyQuote.changePercent.toFixed(1)}%)`);
                if (niftyChain?.strikes?.length) {
                    ...
                    const strikeVals = niftyChain.strikes.map((s) => s.strike);
...
Respond in JSON format:
{
  ...
  "lastUpdated": "${new Date().toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit' })}"
            ...
            if (!briefing.stance)
                briefing.stance = 'neutral';
            if (!briefing.date)
                briefing.date = istDateStr();
    ...
    async getPostTradeBriefing(userId) {
        const todayStart = new Date();
        todayStart.setHours(0, 0, 0, 0);
        ...
        const pnls = trades.map((t) => ({ symbol: t.symbol, pnl: Number(t.netPnl) }));
        const totalPnl = pnls.reduce((sum, p) => sum + p.pnl, 0);
        const winners = pnls.filter((p) => p.pnl > 0).sort((a, b) => b.pnl - a.pnl);
        const losers = pnls.filter((p) => p.pnl < 0).sort((a, b) => a.pnl - b.pnl);
        return {
            date: istDateStr(),
        ...
        try {
            ...
            if (positions.length > 0) {
                ...
                if (isEngineAvailable()) {
                    try {
                        rustRisk = await engineRisk({ returns, initial_capital: initCap });
                    }
    ...
    defaultCapitalRules() {
        return [
            { id: 'max-daily-loss', name: 'Max Daily Loss', status: 'green', detail: 'No trades today' },
            { id: 'position-sizing', name: 'Position Sizing', status: 'green', detail: 'No open positions' },
            { id: 'exposure', name: 'Total Exposure', status: 'green', detail: '0% deployed' },
            { id: 'drawdown-circuit', name: 'Drawdown Circuit', status: 'green', detail: 'No drawdown' },
            { id: 'overtrading-guard', name: 'Overtrading Guard', status: 'green', detail: '0 trades today' },
        ];
    }
}
