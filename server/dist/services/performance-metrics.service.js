...
export class PerformanceMetricsService {
    ...
    async computeDailyMetrics(userId, date) {
        ...
        const strategyPnls = new Map();
        for (const t of trades) {
            const strat = t.strategyTag || 'unknown';
            if (!strategyPnls.has(strat))
                strategyPnls.set(strat, []);
    ...
    async getTargetProgress(userId, targetDailyPct = 0.5) {
        const portfolios = await this.prisma.portfolio.findMany({
            where: { userId },
    ...
    async getMetricsSummary(userId, days = 30) {
...
function computeAvgCorrelation(series) {
    if (series.length < 2)
        return 0;
    // Pad to same length
