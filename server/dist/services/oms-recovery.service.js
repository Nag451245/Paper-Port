...
const TERMINAL_STATUSES = ['FILLED', 'CANCELLED', 'REJECTED', 'EXPIRED'];
...
export class OMSRecoveryService {
    ...
    async recover() {
        ...
        log.info({ count: nonTerminalOrders.length }, 'Non-terminal orders loaded for recovery');
        ...
        for (const order of nonTerminalOrders) {
            ...
            try {
                if (order.status === 'SUBMITTED' &&
                    ...
                    orphanedExpired++;
                    log.warn({ orderId: order.id, ageMs }, 'Orphaned SUBMITTED order auto-expired');
                    continue;
                }
                if (order.status === 'PENDING' &&
                    ...
                    stuckRejected++;
                    log.warn({ orderId: order.id, ageMs }, 'Stuck PENDING order auto-rejected');
                    continue;
                }
    ...
    async gracefulShutdown() {
        ...
        await this.audit.flush();
        log.info({ cancelledCount: pendingOrders.length }, 'Graceful shutdown complete');
    }
}
//# sourceMappingURL=oms-recovery.service.js.map