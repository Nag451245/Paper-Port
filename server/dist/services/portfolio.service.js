...
export class PortfolioService {
    ...
    async create(userId, name, initialCapital) {
    ...
    async getById(portfolioId, userId) {
     ...
     */
    async getSummary(portfolioId, userId, priceCache) {
        ...
        const uncachedPositions = priceCache
            ? openPositions.filter((pos) => !(pos.symbol in priceCache) || priceCache[pos.symbol] <= 0)
            : openPositions;
        const [todayTrades, ltpResults] = await Promise.all([
            ...
            Promise.allSettled(uncachedPositions.map(async (pos) => {
                try {
    ...
    async getEquityCurve(portfolioId, userId) {
    ...
    async getRiskMetrics(portfolioId, userId) {
        await this.getById(portfolioId, userId);
        const trades = await this.prisma.trade.findMany({
            where: { portfolioId },
            orderBy: { exitTime: 'asc' },
    ...
    async getPnlHistory(portfolioId, userId, days = 0) {
        await this.getById(portfolioId, userId);
    ...
    async updateCapital(portfolioId, userId, virtualCapital) {
        const portfolio = await this.getById(portfolioId, userId);
     ...
     */
    async reconcileNav(portfolioId, userId) {
        const portfolio = await this.getById(portfolioId, userId);
        const initialCapital = Number(portfolio.initialCapital);
    ...
    async fetchNiftyDailyReturns(days) {
