...
import { isEngineAvailable, engineScanActiveSymbols, engineOptionsSignals, engineActiveStrategies, enginePerformanceSummary, } from '../lib/rust-engine.js';
const MOOD_TYPES = [
    'COMPOSED',
    'ALERT',
    'FOCUSED',
...
${memoriesContext}`;
    ...
    async getAwareness(userId) {
        try {
            ...
            const todayTrades = val(results[0], []);
            const bots = val(results[1], []);
            const topSignals = val(results[2], []);
            const pendingSignalCount = val(results[3], 0);
            const riskEvents = val(results[4], []);
            const activeTargets = val(results[5], []);
            const latestInsight = val(results[6], null);
            const latestRegime = val(results[7], null);
            const weeklyPnl = val(results[8], []);
            const topStrategies = val(results[9], []);
            let openPositions = 0;
            ...
            for (const p of portfolios) {
                ...
                for (const pos of p.positions) {
                    ...
                    positionDetails.push(`  ${pos.symbol} ${pos.side} @ ₹${entry.toFixed(0)} | P&L: ₹${uPnl.toFixed(0)}${slDist}${tgtDist}`);
                }
            }
            ...
            try {
                if (isEngineAvailable()) {
                    ...
                    const parts = ['Rust Engine: ONLINE'];
                    ...
                    if (opts.length > 0) {
                        const highConf = opts.filter((s) => s.confidence >= 0.5).length;
            ...
            await this.prisma.guardianState.upsert({
                where: { userId },
                update: { awareness: awarenessData },
                create: {
                    userId,
                    mood: 'COMPOSED',
                    moodIntensity: 0.5,
    ...
    // ─── Action-Capable Chat ─────────────────────────────────────────
    async chat(userId, message, _pageContext) {
        try {
            const state = await this.getOrCreateState(userId);
            ...
            if (symbolMatch) {
                ...
                if (symbol.length >= 3 && !reserved.has(symbol)) {
                    memoryOps.push(this.memory.evolveView(userId, symbol, `User discussed ${symbol}: "${message.slice(0, 100)}"`));
                }
            }
    ...
    // ─── Mood Engine ─────────────────────────────────────────────────
    async updateMood(userId, signals) {
        try {
            const state = await this.getOrCreateState(userId);
            let newMood = 'COMPOSED';
            let newIntensity = 0.5;
            if (signals.targetHit) {
                newMood = 'CELEBRATORY';
                newIntensity = 0.8;
            }
            else if ((signals.drawdownPct !== undefined && signals.drawdownPct > 2) ||
                (signals.dayPnlPct !== undefined && signals.dayPnlPct < -0.5)) {
                newMood = 'CAUTIOUS';
                const severity = Math.max(signals.drawdownPct ?? 0, Math.abs(signals.dayPnlPct ?? 0));
                newIntensity = Math.min(1.0, 0.5 + severity * 0.1);
            }
            else if (signals.anomalyDetected ||
                (signals.vix !== undefined && signals.vix > 22)) {
                newMood = 'VIGILANT';
                newIntensity = 0.7;
            }
            else if (signals.regimeChange ||
            ...
            else if (signals.consecutiveLosses !== undefined &&
...
- Match your ${state.mood} mood in tone`;
            ...
            if (mood === 'CAUTIOUS' || mood === 'VIGILANT')
                category = 'alert';
            else if (mood === 'CONTEMPLATIVE' || mood === 'REFLECTIVE')
                category = 'insight';
            ...
            if (mood === 'CAUTIOUS' || mood === 'VIGILANT')
                priority = 'high';
            else if (mood === 'ALERT' || mood === 'FOCUSED')
                priority = 'medium';
            const thought = {
                content,
                category,
    ...
    // ─── Event Handler ───────────────────────────────────────────────
    async onEvent(userId, event) {
        try {
            switch (event.type) {
                case 'trade_executed': {
                    const { symbol, side, pnl } = event.data;
                ...
                case 'regime_change': {
                    ...
                    await this.memory.evolveView(userId, 'market_regime', `Regime shifted to ${regime}. ${event.data.details ?? ''}`);
                ...
                case 'reconciliation_result': {
                    ...
                    if (mismatches > 0) {
                        ...
                        await this.memory.storeMemory(userId, 'trade_lesson', 'reconciliation', `EOD reconciliation found ${mismatches} position mismatches (${matched} matched). Investigate immediately.`, { importance: 0.9 });
                        await this.generateThought(userId);
                    }
                    break;
                }
                case 'exposure_breach': {
                    ...
                    await this.memory.storeMemory(userId, 'trade_lesson', 'risk_management', `${exposureType} exposure limit breached at ${pct.toFixed(1)}%. Order rejected by risk system.`, { importance: 0.7 });
                    break;
                }
                case 'drift_detected': {
                    const psiScore = Number(event.data.psiScore ?? 0);
                    await this.memory.evolveView(userId, 'ml_models', `ML concept drift detected (PSI=${psiScore.toFixed(3)}). Online learning paused until regime stabilizes.`);
                    break;
                }
                default: {
