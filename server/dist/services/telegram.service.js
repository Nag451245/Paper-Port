import { engineScan, engineOptionsSignals, engineOptionsData, isEngineAvailable, enginePerformanceSummary } from '../lib/rust-engine.js';
...
export class TelegramService {
    prisma;
    static pollingInstance = null;
    static pollingStarted = false;
    botToken;
    lastUpdateId = 0;
    polling = false;
    stopRequested = false;
    ...
    async pollUpdates() {
        if (!this.botToken)
            return;
        const res = await fetch(`${TELEGRAM_API}${this.botToken}/getUpdates?offset=${this.lastUpdateId + 1}&timeout=5&allowed_updates=["message"]`, { signal: AbortSignal.timeout(30_000) });
        ...
        for (const update of data.result) {
            ...
            if (!this.markProcessed(update.update_id))
                continue;
            ...
            try {
                ...
                console.error(`[Telegram] Handler error for ${dedupKey}:`, err.message);
            }
            finally {
    ...
    async routeMessage(chatId, text, name) {
        if (text === '/start') {
            await this.sendMessage(chatId, `Namaste ${name}! I'm <b>Chitti</b> — your AI trading companion.\n\n` +
                `Your <b>Chat ID</b> is:\n<code>${chatId}</code>\n\n` +
    ...
    async handleGreeting(chatId, name) {
    ...
    async handleSmartStatus(chatId) {
        ...
        const lines = [`<b>Capital Guard Status</b>\n`];
        ...
        if (bots.length > 0) {
            ...
            if (totalBotPnl !== 0)
                lines.push(`Bot P&L: ${totalBotPnl >= 0 ? '+' : ''}₹${totalBotPnl.toFixed(0)}`);
        }
        lines.push(`Signals (24h): ${recentSignals}`);
        lines.push(`Rust Engine: ${engineOnline ? '<b>ONLINE</b>' : '<b>OFFLINE</b>'}`);
        if (perfSummary) {
            const outcomes = perfSummary.total_outcomes ?? 0;
            const health = perfSummary.avg_health_score ?? 'N/A';
            lines.push(`Performance: ${outcomes} outcomes tracked, avg health: ${health}`);
        }
        await this.sendMessage(chatId, lines.join('\n'));
    }
    async handleHelp(chatId) {
        await this.sendMessage(chatId, `<b>🤖 Chitti — Capital Guard AI</b>\n\n` +
    ...
    async handleScanCommand(chatId, symbol) {
        ...
        try {
            ...
            if (!result.signals || result.signals.length === 0) {
                ...
                await this.sendMessage(chatId, `<b>${symbol} Analysis</b>\n\n` +
            ...
            if (indicators.ema_9 && indicators.ema_21)
                indLines.push(`EMA9: ${indicators.ema_9.toFixed(1)} | EMA21: ${indicators.ema_21.toFixed(1)}`);
            if (indicators.rsi_14)
                indLines.push(`RSI: ${indicators.rsi_14.toFixed(1)}`);
            if (indicators.macd_histogram)
                indLines.push(`MACD Hist: ${indicators.macd_histogram.toFixed(2)}`);
            if (indicators.supertrend)
                indLines.push(`Supertrend: ${indicators.supertrend.toFixed(1)}`);
            await this.sendMessage(chatId, `<b>Rust Engine: ${symbol}</b>\n\n` +
            ...
            await this.sendMessage(chatId, `Scan failed for <b>${symbol}</b>: ${err.message}`);
        }
    }
    async handleOptionsCommand(chatId, symbol) {
        if (!symbol || symbol.length < 2) {
        ...
        try {
            ...
            if (!optData && symbolSignals.length === 0) {
                await this.sendMessage(chatId, `No options data available for <b>${symbol}</b>.\n\n` +
                    `Options data is available for major F&O symbols (NIFTY, BANKNIFTY, RELIANCE, etc.).`);
                return;
            }
            const lines = [`<b>Options Analysis: ${symbol}</b>\n`];
            if (optData) {
            ...
            await this.sendMessage(chatId, `Options analysis failed for <b>${symbol}</b>: ${err.message}`);
        }
    }
    async handleQuoteCommand(chatId, symbol) {
        if (!symbol || symbol.length < 2) {
        ...
        try {
            ...
            await this.sendMessage(chatId, `<b>${changeEmoji} ${symbol}</b>\n\n` +
            ...
            await this.sendMessage(chatId, `Quote failed for <b>${symbol}</b>: ${err.message}`);
        }
    }
    async handleCommandCenterChat(chatId, text) {
        ...
        if (!user) {
            await this.sendMessage(chatId, '🤖 Not connected to any Capital Guard account yet.\n\n' +
        ...
        try {
            ...
            console.error('[Telegram] Command center error:', err.message);
    ...
    async notifyTradeExecution(userId, symbol, side, qty, price, pnl) {
        const emoji = side === 'BUY' ? '🟢' : '🔴';
        const pnlLine = pnl !== undefined ? `\nP&L: <b>${pnl >= 0 ? '+' : ''}₹${pnl.toFixed(2)}</b>` : '';
        return this.notifyUser(userId, `${emoji} Trade Executed`, `${side} ${qty}x ${symbol} @ ₹${price.toFixed(2)}${pnlLine}`);
    }
    async notifySignal(userId, symbol, direction, confidence, entry, target, stopLoss, source) {
        ...
        return this.notifyUser(userId, `${sourceTag} Signal`, `<b>${direction} ${symbol}</b>${entryLine}${targetLine}${slLine}\nConfidence: <b>${(confidence * 100).toFixed(0)}%</b>`);
    }
    async notifyPipelineSignal(symbol, direction, confidence, strategy, mlScore, source) {
        ...
        const text = `<b>Rust Engine Signal</b>\n\n` +
    ...
    async notifyRiskAlert(userId, alertType, message) {
        return this.notifyUser(userId, `Risk Alert: ${alertType}`, message);
    }
    async notifyDailyReport(userId, report) {
        const emoji = report.pnl >= 0 ? '📈' : '📉';
        return this.notifyUser(userId, `${emoji} Daily Report`, `Trades: ${report.trades} | P&L: ${report.pnl >= 0 ? '+' : ''}₹${report.pnl.toFixed(2)}\nWin Rate: ${report.winRate.toFixed(1)}%\nTop Winner: ${report.topWinner}\nTop Loser: ${report.topLoser}\nRegime: ${report.regime}`);
    }
    async verifyConnection(chatId) {
        if (!this.botToken)
            return { success: false };
        try {
            const sent = await this.sendMessage(chatId, '<b>PaperPort Connected!</b>\n\nYou will now receive trade alerts, signals, and daily reports here.');
            return { success: sent };
        }
        catch {
            return { success: false };
