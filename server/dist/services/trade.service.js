...
const TRADING_MODE = (process.env.TRADING_MODE ?? 'PAPER').toUpperCase();
import { calculateCosts } from '../lib/costs.js';
function simulateExecution(idealPrice, qty, side, exchange = 'NSE', orderType = 'MARKET') {
    if (orderType !== 'MARKET') {
        return {
    ...
    const result = fillSim.simulate({ symbol: '', exchange: exchange ?? 'NSE', side, orderType: 'MARKET', qty, price: idealPrice }, marketState);
...
export class TradeService {
    ...
    oms = null;
    _twapExecutor = null;
    fillSimulator = new FillSimulatorService();
    smartRouter;
    executionEngine;
    ...
    isLiveMode() { return TRADING_MODE === 'LIVE' && !!this.broker; }
    getExecutionStats() {
        return {
    ...
    async executeLiveOrder(input) {
        if (!this.broker)
            throw new TradeError('Live broker not configured', 500);
        const brokerInput = {
            symbol: input.symbol,
    ...
    async getTotalInvestedValue(portfolioId) {
        try {
    ...
    async recoverCapital(portfolioId, userId, amountNeeded) {
        const positions = await this.prisma.position.findMany({
            where: { portfolioId, status: 'OPEN' },
            include: { portfolio: true },
        });
        if (positions.length === 0)
            return { recovered: 0, closedPositions: [] };
    ...
    async placeOrder(userId, input, skipMarketCheck = false) {
        try {
        ...
        if (!marketOpen && !skipMarketCheck) {
            throw new TradeError(`Market is closed. Orders cannot be placed or queued outside market hours. ` +
                `${exchange} trading hours: ${exchange === 'MCX' ? '9:00-23:30' : exchange === 'CDS' ? '9:00-17:00' : '9:15-15:30'} IST.`, 400);
        }
        ...
        if (!marketOpen && (input.strategyTag?.startsWith('AI-BOT') || input.strategyTag?.startsWith('BOT:'))) {
            throw new TradeError('STRICT: Bot/Agent orders are blocked outside market hours. No exceptions.', 400);
        }
        ...
        if (redis) {
            ...
            if (!acquired) {
                throw new TradeError(`Another order for ${input.symbol} is already being processed. Please wait.`, 429);
            }
        }
        try {
                    ...
                    if (quote.ltp > 0) {
                        ...
                        if (routing.orderType === 'POV') {
                            ...
                            const result = await povExecutor.execute({
                                ...
                                side: input.side, exchange: input.exchange ?? 'NSE',
                        ...
                        if (routing.orderType === 'IS') {
                            ...
                            const result = await isExecutor.execute({
                                totalQty: input.qty, symbol: input.symbol, side: input.side,
                        ...
                        if (routing.orderType === 'ICEBERG') {
                            ...
                            const result = await icebergExecutor.execute({
                                ...
                                randomizePct: 20, symbol: input.symbol, side: input.side,
                                exchange, portfolioId: input.portfolioId, userId,
            ...
            if (input.orderType === 'MARKET' && fillPrice <= 0) {
                ...
                fillPrice = quote.ltp;
                if (fillPrice <= 0) {
                    throw new TradeError(`Cannot place market order: unable to fetch current price for ${input.symbol}. ` +
                        'Ensure Breeze API session is active or try a limit order with a specific price.', 400);
                }
                ...
                if (quote.timestamp) {
                    ...
                    if (quoteAge > MAX_QUOTE_AGE_MS) {
                        throw new TradeError(`Price for ${input.symbol} is stale (${Math.round(quoteAge / 1000)}s old). ` +
                            'Cannot place market order with outdated price data. Retry or use a limit order.', 400);
                    }
                }
            }
            let brokerOrderId;
            ...
            if (this.isLiveMode()) {
                ...
                if (this.broker && liveResult.orderId) {
                    ...
                    for (let poll = 0; poll < MAX_POLLS; poll++) {
                        ...
                        if (status.filledQty > 0)
                            effectiveQty = status.filledQty;
                        if (terminalStates.has(status.status)) {
                            if (status.status === 'REJECTED' || status.status === 'CANCELLED') {
                                throw new TradeError(`Broker ${status.status.toLowerCase()} the order: ${status.message ?? 'Unknown reason'}`, 400);
                            }
                            break;
                        }
                        if (poll < MAX_POLLS - 1) {
                ...
                const execSim = simulateExecution(fillPrice, input.qty, input.side, exchange, input.orderType);
            ...
            if (input.side === 'BUY') {
                ...
                if (!existingShort) {
                    if (totalValue > availableCash) {
                        throw new TradeError(`Insufficient capital. Need ₹${totalValue.toFixed(0)} but only ₹${availableCash.toFixed(0)} available.`, 400);
                    }
                    if (totalInvested + totalValue > declaredCapital * 1.0) {
                        throw new TradeError(`STRICT: This order (₹${totalValue.toFixed(0)}) would push total invested (₹${totalInvested.toFixed(0)}) ` +
                            `beyond declared capital of ₹${declaredCapital.toFixed(0)}. Not a single rupee more.`, 400);
                    }
                }
            }
            else {
                ...
                if (!existingLong) {
                    ...
                    if (marginRequired > availableCash) {
                        throw new TradeError(`Insufficient margin for short. Need ₹${marginRequired.toFixed(0)} but only ₹${availableCash.toFixed(0)} available.`, 400);
                    }
                    if (totalInvested + marginRequired > declaredCapital * 1.0) {
                        throw new TradeError(`STRICT: Short margin (₹${marginRequired.toFixed(0)}) would push total utilization (₹${totalInvested.toFixed(0)}) ` +
                            `beyond declared capital of ₹${declaredCapital.toFixed(0)}. Not a single rupee more.`, 400);
                    }
                }
            }
    ...
    shortMarginRequired(price, qty, exchange) {
        const rate = exchange === 'MCX' ? 0.10 : exchange === 'CDS' ? 0.05 : 0.25;
        return price * qty * rate;
    }
    async safeUpdateNav(portfolioId, currentNav, delta, db) {
    ...
    async handleFill(orderId, input, fillPrice, costs) {
    ...
    async handleBuyFill(orderId, input, fillPrice, costs, db) {
        const prisma = db ?? this.prisma;
    ...
    async openLongPosition(orderId, input, fillPrice, costs, qty, db) {
        const prisma = db ?? this.prisma;
    ...
    async handleSellFill(orderId, input, fillPrice, costs, db) {
        const prisma = db ?? this.prisma;
        const existingLong = await prisma.position.findFirst({
            where: { portfolioId: input.portfolioId, symbol: input.symbol, side: 'LONG', status: 'OPEN' },
        });
    ...
    async openShortPosition(orderId, input, fillPrice, costs, qty, db) {
        if (fillPrice <= 0) {
    ...
    async listOrders(userId, params = {}) {
    ...
    async listPositions(userId, strategyTag) {
        const portfolios = await this.prisma.portfolio.findMany({
            where: { userId },
            select: { id: true },
        });
        const portfolioIds = portfolios.map((p) => p.id);
        const where = { portfolioId: { in: portfolioIds }, status: 'OPEN' };
        if (strategyTag)
            where.strategyTag = strategyTag;
        return this.prisma.position.findMany({
            where,
            orderBy: { openedAt: 'desc' },
        });
    }
    async listActiveStrategies(userId) {
        const portfolios = await this.prisma.portfolio.findMany({
            where: { userId },
            select: { id: true },
        });
    ...
    async exitStrategyLegs(userId, positionIds) {
        const results = [];
    ...
    async closePosition(positionId, userId, exitPrice) {
        ...
        const exitOrder = await this.prisma.order.create({
            data: {
                ...
                instrumentToken: position.instrumentToken ?? `${position.symbol}-EXIT`,
    ...
    async listTrades(userId, params = {}) {
        ...
        if (fromDate || toDate) {
            where.exitTime = {};
            if (fromDate)
                where.exitTime.gte = new Date(fromDate);
            if (toDate) {
        ...
        for (const order of pendingOrders) {
            try {
                ...
                if (order.orderType === 'MARKET') {
                    ...
                    if (order.side === 'BUY' && ltp <= orderPrice)
                        shouldFill = true;
                    if (order.side === 'SELL' && ltp >= orderPrice)
                        shouldFill = true;
                }
                if (!shouldFill)
                    continue;
