...
export class LessonsEngineService {
    generateLesson(memory) {
        ...
        return (`${memory.symbol} ${memory.signalDirection} via ${memory.signalStrategy} ` +
    ...
    async generatePatternInsights(userId) {
    ...
    async getRelevantLessons(userId, symbol, strategy, regime, limit = 10) {
        const prisma = getPrisma();
        const memories = await prisma.marketMemory.findMany({
            where: {
                userId,
        ...
        for (const { memory } of scored.slice(0, limit)) {
            if (memory.outcome == null || memory.pnlPct == null || memory.holdingMinutes == null)
                continue;
            const resolved = {
    ...
    regimeMatchesDirection(regime, direction) {
        const r = regime.toUpperCase().replace(/\s+/g, '_');
        return ((direction === 'BUY' && r === 'TRENDING_UP') ||
            (direction === 'SELL' && r === 'TRENDING_DOWN'));
    }
    regimeContradictsDirection(regime, direction) {
        const r = regime.toUpperCase().replace(/\s+/g, '_');
        return ((direction === 'BUY' && r === 'TRENDING_DOWN') ||
            (direction === 'SELL' && r === 'TRENDING_UP'));
    }
    detectGapDayPatterns(memories, patterns) {
        for (let dow = 0; dow < 7; dow++) {
            const gapTrades = memories.filter(m => m.dayOfWeek === dow && Math.abs(m.gapPct) > 0.5);
            if (gapTrades.length < 5)
                continue;
            for (const strategy of this.uniqueValues(gapTrades, 'signalStrategy')) {
                ...
                const avgPnl = subset.reduce((s, m) => s + (m.pnlPct ?? 0), 0) / subset.length;
    ...
    detectNiftyBandPatterns(memories, patterns) {
    ...
    detectStrategyRegimePatterns(memories, patterns) {
    ...
    detectSymbolLevelPatterns(memories, patterns) {
        ...
        for (const symbol of symbols) {
            ...
            if (keyLevelTrades.length < 5)
                continue;
            const wins = keyLevelTrades.filter(m => m.outcome === 'WIN').length;
            const winRate = wins / keyLevelTrades.length;
            const avgPnl = keyLevelTrades.reduce((s, m) => s + (m.pnlPct ?? 0), 0) / keyLevelTrades.length;
            patterns.push({
    ...
    detectTimeOfDayPatterns(memories, patterns) {
        const timeSlots = [
        ...
        for (const slot of timeSlots) {
            for (const strategy of this.uniqueValues(memories, 'signalStrategy')) {
                const subset = memories.filter(m => m.hourOfDay >= slot.min && m.hourOfDay < slot.max && m.signalStrategy === strategy);
                if (subset.length < 5)
                    continue;
                const wins = subset.filter(m => m.outcome === 'WIN').length;
                const winRate = wins / subset.length;
    ...
    detectVixPatterns(memories, patterns) {
        const vixBands = [
        ...
        for (const vb of vixBands) {
            for (const strategy of this.uniqueValues(memories, 'signalStrategy')) {
                const subset = memories.filter(m => m.vixLevel >= vb.min && m.vixLevel < vb.max && m.signalStrategy === strategy);
                if (subset.length < 5)
                    continue;
                const wins = subset.filter(m => m.outcome === 'WIN').length;
                const winRate = wins / subset.length;
    ...
    patternConfidence(occurrences, winRate) {
        const deviation = Math.abs(winRate - 0.5);
        if (occurrences >= 20 && deviation > 0.15)
            return 'HIGH';
        if (occurrences >= 10 && deviation > 0.10)
            return 'MEDIUM';
        return 'LOW';
    }
    uniqueValues(records, field) {
        ...
        for (const r of records) {
            if (r[field] != null)
                set.add(String(r[field]));
        }
        return Array.from(set);
    }
}
//# sourceMappingURL=lessons-engine.service.js.map