...
export class HistoricalDataService {
    async fetchAndStore(symbol, exchange, timeframe, startDate, endDate) {
    ...
    async getHistory(symbol, exchange, timeframe, startDate, endDate) {
        const prisma = getPrisma();
    ...
    adjustForCorporateActions(symbol, bars, actions) {
        if (actions.length === 0)
            return bars;
    ...
    async getAdjustedSeries(symbol, exchange, startDate, endDate) {
    ...
    async recordCorporateAction(symbol, actionType, ratio, exDate, details) {
        const prisma = getPrisma();
