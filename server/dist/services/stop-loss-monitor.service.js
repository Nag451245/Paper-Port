...
export class StopLossMonitor {
    ...
    async start() {
        ...
        this.intervalHandle = setInterval(() => {
            this.runChecks().catch(err => log.error({ err }, 'Check cycle error'));
        }, this.checkIntervalMs);
        this.reloadHandle = setInterval(() => {
            this.syncOpenPositions().catch(err => log.warn({ err }, 'Position reload error'));
        }, RELOAD_INTERVAL_MS);
    }
    stop() {
        if (this.intervalHandle) {
    ...
    addPosition(config) {
        if (this.monitoredPositions.has(config.positionId))
            return;
    ...
    updateStopLoss(positionId, newStopPrice) {
    ...
    async runChecks() {
        ...
        for (const [posId, monitored] of this.monitoredPositions) {
            ...
            // Update watermarks
            if (ltp > monitored.highWaterMark)
                monitored.highWaterMark = ltp;
            if (ltp < monitored.lowWaterMark || monitored.lowWaterMark === 0)
                monitored.lowWaterMark = ltp;
        ...
        if (pnlUpdates.length > 0) {
            await Promise.allSettled(pnlUpdates.map(({ id, unrealizedPnl }) => this.prisma.position.update({
                where: { id },
                data: { unrealizedPnl },
            }).catch(err => log.warn({ err, positionId: id }, 'Failed to persist unrealizedPnl'))));
        }
    }
    async executeStopLossExit(monitored, ltp, reason) {
        const { config } = monitored;
        log.info({ symbol: config.symbol, reason, ltp }, 'EXIT triggered');
        const decisionType = reason.includes('Take-profit') ? 'TP_TRIGGER'
            : reason.includes('Time-based') ? 'EXIT_SIGNAL'
                : 'SL_TRIGGER';
...
//# sourceMappingURL=stop-loss-monitor.service.js.map