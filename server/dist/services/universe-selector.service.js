...
export class UniverseSelectorService {
    ...
    async select(criteria) {
        ...
        for (const bar of recentBars) {
            if (!barsBySymbol.has(bar.symbol))
                barsBySymbol.set(bar.symbol, []);
        ...
        for (const tick of recentTicks) {
            if (tick.bid != null && tick.ask != null && tick.ltp > 0) {
                if (!spreadBySymbol.has(tick.symbol))
                    spreadBySymbol.set(tick.symbol, []);
        ...
        for (const [symbol, bars] of barsBySymbol) {
            ...
            if (criteria.minAvgVolume && avgVolume < criteria.minAvgVolume)
                continue;
            ...
            if (criteria.maxSpreadBps && avgSpreadBps > criteria.maxSpreadBps)
                continue;
            ...
            if (avgSpreadBps > 0)
                reasons.push(`spread=${avgSpreadBps.toFixed(1)}bps`);
    ...
    async refreshUniverse(userId, criteria) {
...
//# sourceMappingURL=universe-selector.service.js.map