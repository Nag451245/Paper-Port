import { engineWalkForward } from '../lib/rust-engine.js';
export class StrategyComposer {
    prisma;
    constructor(prisma) {
        this.prisma = prisma;
    }
    async composePortfolio(userId) {
        ...
        let totalWeight = 0;
        for (const [sid, stats] of strategyStats) {
            if (stats.sharpe < -0.5 || stats.trades < 3)
                continue;
    ...
    async validateWithWalkForward(userId, strategyId, candles) {
    ...
    getCorrelationGroup(strategyId) {
        const trendFollowing = ['ema-crossover', 'supertrend', 'sma_crossover', 'momentum'];
        const meanReversion = ['mean_reversion', 'rsi_reversal'];
        const breakout = ['orb'];
        if (trendFollowing.includes(strategyId))
            return 'trend_following';
        if (meanReversion.includes(strategyId))
            return 'mean_reversion';
        if (breakout.includes(strategyId))
            return 'breakout';
        return 'other';
    }
    computeDiversification(strategies) {
    ...
    async defaultComposition(userId) {
        const portfolio = await this.prisma.portfolio.findFirst({ where: { userId } });
        const totalCapital = portfolio ? Number(portfolio.currentNav) : 1000000;
        const defaults = [
...
//# sourceMappingURL=strategy-composer.js.map