...
import { engineScan, engineRisk, engineSignals, isEngineAvailable, engineScanActiveSymbols, engineOptionsSignals, engineCalibrateConfidence, engineExecutionPlan } from '../lib/rust-engine.js';
import { calculateMaxPain, calculateIVPercentile } from './options.service.js';
...
import { StrategyRegistry } from './strategy-sdk.js';
...
export class BotEngine {
    ...
    cycleInProgress = new Set();
    rollingAccuracy = new Map();
    tickInterval = DEFAULT_TICK_INTERVAL;
    signalInterval = DEFAULT_SIGNAL_INTERVAL;
    marketScanInterval = DEFAULT_MARKET_SCAN_INTERVAL;
    targetTracker;
    globalMarket;
    decisionAudit;
    calendar;
    riskService;
    twapExecutor;
    portfolioOptimizer = new PortfolioOptimizerService();
    cachedStrategyParams = new Map();
    paramsLastLoaded = 0;
    // Online learning state
    strategyBeta = new Map();
    recentWinRates = new Map();
    intradayVolatility = new Map();
    mlWeights = null;
    abTestTracker = { rustCorrect: 0, pythonCorrect: 0, total: 0 };
    featurePipeline = new FeaturePipelineService();
    marketMemory = new MarketMemoryService();
    decisionFusion = new DecisionFusionService();
     ...
     */
    async loadMLWeightsFromDB(userId) {
        try {
            const whereClause = { strategyId: 'ml_scorer_weights', isActive: true };
            if (userId)
     ...
     */
    thompsonSelectStrategy(availableStrategies) {
    ...
    async loadAlphaDecayState(userId) {
        try {
     ...
     */
    computeDynamicStopLoss(symbol, baseStopPct) {
     ...
     */
    updateIntradayVolatility(symbol, recentReturns) {
    ...
    async loadStrategyParams(userId) {
        const now = Date.now();
     ...
     */
    deriveGateScores(confidence, indicators, votes, extra) {
        ...
        const adx = ind.adx ?? 0;
        const g1 = Math.min(100, Math.max(0, adx > 0 ? adx + (emaCross > 0 ? 20 : -10) + (superTrend > 0 ? 15 : -5)
            : c + (emaCross > 0 ? 10 : -10)));
        // G2 Momentum: RSI, MACD, Stochastic
        const rsi = ind.rsi ?? 50;
        const macdVote = v.macd ?? 0;
        const g2 = Math.min(100, Math.max(0, (rsi > 50 ? rsi - 10 : 100 - rsi - 10) + (macdVote > 0 ? 25 : macdVote < 0 ? -10 : 5)));
        // G3 Volatility: ATR, Bollinger, VIX
        const atr = ind.atr ?? 0;
        const vix = ind.vix ?? 15;
        const bbVote = v.bollinger ?? 0;
        const g3 = Math.min(100, Math.max(0, vix < 20 ? 70 + (bbVote > 0 ? 15 : -5) : vix < 30 ? 50 : 30));
    ...
    getRollingAccuracy(strategyId) {
        return this.rollingAccuracy.get(strategyId);
    }
    trackOutcome(strategyId, outcome) {
    ...
    async checkAutoPause(userId, strategyId, botId) {
        const entry = this.rollingAccuracy.get(strategyId);
        if (!entry || entry.outcomes.length < 5)
            return false;
    ...
    async startBot(botId, userId) {
        ...
        setTimeout(() => {
            this.runBotCycle(botId, userId).catch(err => {
                console.error(`[BotEngine] Initial cycle failed for bot ${botId}:`, err.message);
            });
        }, staggerMs);
        const timer = setInterval(() => {
            if (!this.cycleInProgress.has(botId)) {
                this.runBotCycle(botId, userId).catch(err => {
                    console.error(`[BotEngine] Cycle failed for bot ${botId}:`, err.message);
                });
            }
        }, this.tickInterval);
    ...
    async startAgent(userId) {
        ...
        setTimeout(() => {
            this.runAgentCycle(userId).catch(err => {
                console.error(`[BotEngine] Initial agent cycle failed:`, err.message);
            });
        }, 20_000);
        const timer = setInterval(() => {
            this.runAgentCycle(userId).catch(err => {
                console.error(`[BotEngine] Agent cycle failed:`, err.message);
            });
        }, this.signalInterval);
    ...
    stopAll() {
        for (const [id] of this.runningBots)
            this.stopBot(id);
        for (const [id] of this.runningAgents)
            this.stopAgent(id);
        this.stopMarketScan();
    }
    get killSwitchActive() { return this._killSwitchActive; }
    activateKillSwitch() {
    ...
    async detectCurrentRegime(candleData) {
        try {
            if (candleData.length === 0 || candleData[0].candles.length < 20)
                return null;
            const bars = candleData[0].candles.map((c) => ({
    ...
    async runMarketScan(userId) {
        ...
        try {
            ...
            const allMovers = [
                ...gainers.map(m => ({ ...m, moverType: 'gainer' })),
                ...losers.map(m => ({ ...m, moverType: 'loser' })),
            ];
            ...
            if (this.rustAvailable) {
                ...
                for (const mover of uniqueSymbols.slice(0, MAX_CANDLE_SYMBOLS * 2)) {
                    try {
                        const bars = await this.marketData.getHistory(mover.symbol, '5m', fromDate, toDate, userId);
                        if (bars.length >= 26) {
                            candleData.push({
                                symbol: mover.symbol,
                ...
                if (candleData.length > 0) {
                    ...
                    signals = rustSignals.map(sig => {
                        ...
                        let valid = true;
                        if (liveLtp > 0) {
                            if (sig.direction === 'BUY' && effectiveTarget <= liveLtp)
                                valid = false;
                            if (sig.direction === 'SELL' && effectiveTarget >= liveLtp)
                                valid = false;
                        }
                        return {
                            symbol: sig.symbol,
                            ...
                            votes: sig.votes,
                            moverType: mover?.moverType ?? 'gainer',
                            _valid: valid,
                        };
            ...
            if (signals.length === 0 && uniqueSymbols.length > 0) {
                try {
                    ...
                    const moverSummary = topMovers.map(m => {
                        ...
                        return `${m.symbol} (${m.moverType}): ₹${m.ltp.toFixed(2)} (${m.changePercent >= 0 ? '+' : ''}${m.changePercent.toFixed(1)}%) | Open: ₹${m.open.toFixed(2)} | PrevClose: ₹${m.previousClose.toFixed(2)} | ${dayRange} | ${volStr}`;
                    }).join('\n');
...
IMPORTANT: Keep each reason under 30 words. Return at most 5 signals. No extra text outside JSON.` },
            ...
            for (const sig of signals.filter(s => s.confidence >= 0.65)) {
                try {
                    const scanGateScores = this.deriveGateScores(sig.confidence, sig.indicators, sig.votes, { source: this.rustAvailable ? 'market-scanner' : 'gpt-market-scanner' });
    ...
    resolveStrategyTag(signalSource) {
        switch (signalSource) {
    ...
    async executeTrade(userId, symbol, direction, rationale, botId, signalMeta) {
        try {
            ...
            const parsedIndicators = signalMeta?.indicators ? (() => {
                try {
                    return JSON.parse(signalMeta.indicators);
                }
                catch {
                    return {};
                }
            ...
            if (direction === 'BUY') {
                ...
                try {
                    ...
                    const optimized = this.portfolioOptimizer.optimizePosition({ symbol, expectedReturn: (signalMeta?.confidence ?? 0.5) * 0.02, confidence: signalMeta?.confidence ?? 0.5, direction, currentWeight: 0 }, { capital: nav, positions: positions.map(p => ({ symbol: p.symbol, side: p.side, qty: p.qty, avgPrice: Number(p.avgEntryPrice) })), currentPrice: ltp > 0 ? ltp : nav * kellyAllocation });
                ...
                if (!riskCheck.allowed) {
                    ...
                    if (auditId) {
                        try {
                            await this.decisionAudit.resolveDecision(auditId, { exitPrice: 0, pnl: 0, predictionAccuracy: 0, outcomeNotes: msg });
                        }
                        catch { }
                    }
                    try {
                ...
                if (longPosition) {
                    ...
                    if (exitPrice <= 0)
                        return { success: false, message: `Cannot sell ${symbol}: no price available` };
                    ...
                    const outcome = Math.abs(pnl) < 10 ? 'BREAKEVEN' : pnl > 0 ? 'WIN' : 'LOSS';
                ...
                if (ltp <= 0)
                    return { success: false, message: `Cannot short ${symbol}: no price available` };
                const kellyAllocation = await this.computeKellySize(userId, symbol, nav);
                const maxPerTrade = nav * kellyAllocation;
    ...
    async executeMultiLegStrategy(userId, symbol, strategyName, legs, botId) {
        try {
            const portfolio = await this.prisma.portfolio.findFirst({ where: { userId } });
            if (!portfolio)
                return { success: false, message: 'No portfolio found' };
    ...
    async getLotSizeForSymbol(symbol) {
        try {
        ...
        const defaults = { NIFTY: 65, BANKNIFTY: 30, FINNIFTY: 60, MIDCPNIFTY: 120 };
    ...
    detectExchange(symbol) {
        ...
        if (mcxSymbols.includes(upper))
            return 'MCX';
        if (cdsSymbols.includes(upper))
            return 'CDS';
        return 'NSE';
    }
    cachedAllocations = null;
    allocationsLastLoaded = 0;
    async computeKellySize(userId, symbol, nav, strategyTag) {
        try {
    ...
    async updateBotTradeStats(botId, pnl) {
        if (pnl === 0)
            return;
        try {
            const bot = await this.prisma.tradingBot.findUnique({ where: { id: botId } });
    ...
    async fetchCandles(symbols, userId) {
    ...
    async runBotCycle(botId, userId) {
        if (this.cycleInProgress.has(botId))
            return;
                ...
                if (candleData.length > 0) {
                    ...
                    for (const cd of candleData) {
                        if (cd.candles && cd.candles.length >= 10) {
                            const recentCloses = cd.candles.slice(-20).map((c) => c.close);
                            const rets = [];
                    ...
                    try {
                        ...
                        console.error(`[BotEngine] Bot ${botId}: Rust scan error:`, err.message);
                        rustSignals = [];
                    }
                    if (rustSignals.length > 0) {
     ...
     */
    async pyramidWinners(bot, userId, botId) {
        try {
            ...
            for (const pos of openPositions) {
                ...
                if (existingOrders >= 3)
                    continue; // original + 2 pyramids max
    ...
    async handleRustSignals(rustSignals, bot, userId, botId, detectedRegime) {
        const shouldAutoExecute = bot.role === 'EXECUTOR' || bot.role === 'SCANNER';
        const openPositions = await this.prisma.position.findMany({
            where: { portfolio: { userId }, status: 'OPEN' },
            select: { symbol: true, side: true },
        });
        const heldSymbols = new Map();
        ...
        const filtered = rustSignals.filter(sig => {
            ...
            if (!held)
                return true;
            if (sig.direction === 'BUY' && held === 'LONG')
                return false;
            if (sig.direction === 'SELL' && held === 'SHORT')
                return false;
            return true;
        });
        ...
        for (const sig of prioritized.slice(0, 5)) {
            ...
            let fusionDecision = null;
            let mlScoreResultOuter = { winProbability: 0.5, confidence: 0, available: false };
            try {
                const candles = await this.marketData.getHistory(sig.symbol, '1d', new Date(Date.now() - 250 * 86400000).toISOString().split('T')[0], new Date().toISOString().split('T')[0], undefined, 'NSE');
                ...
                let mlScoreResult = { winProbability: 0.5, confidence: 0, available: false };
                try {
                    if (await isMLServiceAvailable()) {
                        ...
                        const tftSequence = candleBars.slice(-30).map((b, i, arr) => ({
                            ...b,
                ...
                const lessons = await this.lessonsEngine.getRelevantLessons(userId, sig.symbol, sig.strategy ?? 'default', detectedRegime ?? 'MEAN_REVERTING', 3);
                fusionDecision = await this.decisionFusion.decide({ symbol: sig.symbol, direction: sig.direction, confidence: sig.confidence, entry: sig.entry, stopLoss: sig.stop_loss, target: sig.target, indicators: sig.indicators ?? {}, votes: sig.votes ?? {}, strategy: sig.strategy }, mlScoreResult, { ...memoryRecall, lessons }, { current: detectedRegime ?? 'MEAN_REVERTING', confidence: detectedRegime ? 0.7 : 0.5, source: detectedRegime ? 'rule_based' : 'rule_based' }, strategyHealth, userId);
                await this.marketMemory.recordMemory({
            ...
            if (!execute) {
                ...
                if (!shouldAutoExecute)
                    reasons.push(`bot role '${bot.role}' cannot auto-execute (needs EXECUTOR/SCANNER)`);
                if (fusionDecision && fusionDecision.action !== 'EXECUTE')
                    reasons.push(`DecisionFusion action=${fusionDecision.action}`);
                if (!fusionDecision && finalConfidence < 0.35)
                    reasons.push(`confidence ${(finalConfidence * 100).toFixed(0)}% < 35% threshold`);
    ...
    async executePendingSignals(botId, userId, symbols) {
        try {
            ...
            for (const sig of pendingSignals) {
                const result = await this.executeTrade(userId, sig.symbol, sig.signalType, sig.rationale ?? '', botId, {
            ...
            console.log(`[BotEngine] Bot ${botId}: executePendingSignals error: ${err.message}`);
        }
    }
    async runSDKStrategy(strategyName, symbol, userId, exchange = 'NSE') {
        try {
            const registry = StrategyRegistry.getInstance();
            const strategy = registry.get(strategyName);
            if (!strategy)
                return null;
            const candles = await this.marketData.getHistory(symbol, '1d', new Date(Date.now() - 250 * 86400000).toISOString().split('T')[0], new Date().toISOString().split('T')[0], undefined, exchange);
    ...
    async gptValidateSignal(sig, bot, _userId) {
...
Approve or reject?` },
    ...
    async mlValidateSignal(sig) {
        const now = new Date();
        let rustScore = -1;
        let pythonScore = -1;
        let usedModel = 'confidence';
        const featureObj = {
        ...
        if (this.abTestTracker.total % 50 === 0 && this.abTestTracker.total > 0) {
            try {
                ...
                if (recentABTests.length >= 30 && lgbWins > xgbWins + 5) {
                    log.info({ xgbWins, lgbWins, samples: recentABTests.length }, 'Challenger model (LightGBM) outperforming — promoting as primary');
                    emit('system', {
        ...
        if (pythonScore >= 0)
            return pythonScore >= 0.5;
        if (rustScore >= 0)
            return rustScore >= 0.5;
        return sig.confidence >= 0.45;
    }
    async fetchFnOContext(symbols) {
        const parts = [];
        try {
            const vixData = await this.marketData.getVIX().catch(() => null);
            if (vixData?.value)
                parts.push(`India VIX: ${vixData.value}`);
        }
        catch { /* skip */ }
        for (const sym of symbols.slice(0, 3)) {
            try {
                ...
                const strikeVals = chain.strikes.map((s) => s.strike);
                ...
                const ivValues = chain.strikes.map((s) => s.callIV || s.putIV || 0).filter((v) => v > 0);
    ...
    async computeRustIndicators(symbols, userId) {
        if (!this.rustAvailable)
            return '';
        const candleData = await this.fetchCandles(symbols, userId);
        if (candleData.length === 0)
        ...
        for (const { symbol, candles } of candleData.slice(0, 5)) {
            try {
                const result = await engineSignals({ candles });
                if (!result)
                    continue;
    ...
    async runGptBotCycle(botId, userId, bot, symbols) {
        ...
        try {
            ...
            console.error(`[BotEngine] GPT cycle: quote fetch failed:`, err.message);
...
7. Include both BUY and SELL signals as appropriate for the market condition` },
        ...
        if (analysis.signals && analysis.signals.length > 0) {
            ...
            for (const sig of analysis.signals.slice(0, 5)) {
                ...
                const rationale = sig.entry
                    ...
                    : sig.reason;
                const botGateScores = this.deriveGateScores(sig.confidence, undefined, undefined, { source: 'gemini-analysis', riskReward: sig.riskReward });
                await this.prisma.aITradeSignal.create({
                    data: {
                        userId,
                        symbol: sig.symbol,
                        ...
                        rationale: isMultiLeg
                            ? `${sig.strategy || sig.direction}: ${sig.legs.map((l) => `${l.action} ${l.type}@${l.strike}`).join(' + ')} | ${rationale}`
                            : rationale,
                ...
                const signalMsg = isMultiLeg
                    ? `${sig.strategy || sig.direction} ${sig.symbol}: ${sig.legs.map((l) => `${l.action} ${l.qty || 1}x ${l.type}@${l.strike}`).join(' + ')} | Conf: ${(sig.confidence * 100).toFixed(0)}% | ${sig.reason}`
                ...
                if (shouldExecute) {
                    ...
                    if (isMultiLeg) {
                        ...
                        result = await this.executeTrade(userId, sig.symbol, sig.direction, rationale, botId, {
                            confidence: sig.confidence,
                            ltp: sig.entry,
    ...
    async runAgentCycle(userId) {
        try {
            ...
            if (this.rustAvailable) {
                ...
                if (candleData.length > 0) {
                    ...
                    for (const sig of rustSignals.slice(0, 5)) {
                        if (sig.confidence < (config.minSignalScore || 0.30))
                            continue;
                        if (sig.direction !== 'BUY' && sig.direction !== 'SELL')
                            continue;
                        if (riskData && riskData.max_drawdown_percent > 10)
                            continue;
            ...
            const posInfo = positions.map(p => `${p.symbol} | ${p.side} ${p.qty}@₹${Number(p.avgEntryPrice).toFixed(1)} | P&L: ₹${Number(p.unrealizedPnl).toFixed(0)}`).join('\n') || 'No open positions';
            const result = await chatCompletionJSON({
                messages: [
...
Scan and generate signals. Both BUY and SELL are valid — stocks can be shorted. Keep reasons concise (<30 words each).` },
            ...
            console.error(`[BotEngine] Agent cycle error:`, err.message);
        }
    }
    computePortfolioReturns(positions) {
        const returns = [];
    ...
    async getTargetContextString(userId) {
        try {
    ...
    async getPortfolioPositions(userId) {
        try {
            ...
            if (positions.length === 0)
                return '';
            return positions.map(p => `${p.symbol} | ${p.side} ${p.qty}@₹${Number(p.avgEntryPrice).toFixed(1)} | Unrealized: ₹${Number(p.unrealizedPnl).toFixed(0)}`).join('\n');
        }
        catch {
            return '';
        }
    ...
    async getStrategyHealth(userId, strategyTag) {
        try {
