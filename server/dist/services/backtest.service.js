...
function runORB(bars, params, initialCapital) {
    ...
    for (let i = 1; i < bars.length; i++) {
        ...
        if (rangeRatio > 0.05)
            continue; // skip very volatile days
...
function runSMACrossover(bars, params, initialCapital) {
    ...
    for (let i = 1; i < bars.length; i++) {
        ...
        if (s == null || l == null || ps == null || pl == null)
            continue;
...
function runMeanReversion(bars, params, initialCapital) {
    ...
    let capital = initialCapital;
    let inTrade = false;
    let entryPrice = 0, entryDate = '', entryQty = 0, side = 'LONG';
...
function runMomentum(bars, params, initialCapital) {
...
function runRSIReversal(bars, params, initialCapital) {
...
function computeMetrics(trades, initialCapital, equityCurve) {
...
export class BacktestService {
    ...
    async run(userId, input) {
        const bars = await this.marketService.getHistory(input.symbol, '1d', input.startDate, input.endDate, userId);
        if (bars.length < 5) {
            throw new BacktestError(`Insufficient historical data for ${input.symbol} (got ${bars.length} bars). Please configure your Breeze API key and session token in Settings.`, 422);
        }
        ...
        if (useRust) {
            try {
                ...
                trades = (rustResult.trade_log ?? []).map((t) => ({
                ...
                equityCurve = (rustResult.equity_curve ?? []).map((p) => ({
    ...
    async compare(userId, resultIds) {
        return this.prisma.backtestResult.findMany({
