...
function minutesUntilMarketClose() {
...
export class TargetTracker {
    ...
    async createTarget(userId, data) {
    ...
    async getRecentPnlRecords(userId, days = 7) {
        const from = new Date();
    ...
    computeAggression(currentPnl, targetAbs, maxLossAbs, status, consecutiveLossDays) {
        if (status === 'LOSS_LIMIT' || status === 'REVIEW_REQUIRED')
            return 'none';
        if (status === 'TARGET_HIT')
            return 'low';
        if (consecutiveLossDays >= 2)
            return 'none';
        if (currentPnl <= -(maxLossAbs * 0.7))
            return 'none';
        if (currentPnl <= -(maxLossAbs * 0.5))
            return 'low';
        ...
        if (remaining > 120)
            return 'high';
        if (remaining > 60 && progressRatio < 0.3)
            return 'high';
        return 'medium';
    }
    isTradingAllowed(status, consecutiveLossDays, lastReviewDate) {
...
//# sourceMappingURL=target-tracker.service.js.map