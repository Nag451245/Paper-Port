...
function getNextExpiryForSymbol(symbol) {
    const now = new Date();
    const sym = (symbol ?? '').toUpperCase();
    const fmt = (d) => d.toISOString().split('T')[0] + 'T06:00:00.000Z';
...
async function yahooChart(symbol) {
    try {
        const ac = new AbortController();
        const t = setTimeout(() => ac.abort(), 8000);
        const res = await fetch(`https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?range=1d&interval=1d`, { headers: { 'User-Agent': UA }, signal: ac.signal });
        clearTimeout(t);
        if (!res.ok) {
            await drainResponse(res);
            return null;
...
async function yahooChartBatch(symbols) {
    ...
    for (const chunk of chunks) {
        ...
        for (let i = 0; i < chunk.length; i++) {
            if (batch[i])
                results.set(chunk[i], batch[i]);
        }
    }
    return results;
}
async function niftyTraderFiiDii() {
    try {
        const ac = new AbortController();
...
export class IntelligenceService {
    cache;
    chainInflight = new Map();
    constructor(cache) {
        this.cache = cache ?? null;
    }
    fetchOptionsChainDeduped(symbol) {
    ...
    async getFIIDIITrend(days = 30) {
        return this.cached(`intel:fii-dii-trend:${days}`, async () => {
            ...
            try {
                const rows = await niftyTraderFiiDii();
                if (rows && rows.length > 0) {
                    return rows.slice(0, days).reverse().map((r) => ({
        ...
        return this.cached(`intel:iv-percentile:${symbol}`, async () => {
            try {
                ...
                if (chain?.strikes?.length > 0) {
                    ...
                    let currentIV = 0;
                    if (spot > 0) {
                        const atm = strikes.reduce((best, s) => Math.abs(s.strike - spot) < Math.abs(best.strike - spot) ? s : best, strikes[0]);
                        currentIV = Math.max(atm.callIV ?? 0, atm.putIV ?? 0);
                    }
                    if (currentIV <= 0) {
                        const ivs = strikes.map((s) => Math.max(s.callIV ?? 0, s.putIV ?? 0)).filter((v) => v > 0);
                    ...
                    if (currentIV > 0) {
                        ...
                        const atmIdx = spot > 0
                            ? strikes.reduce((bi, s, i) => Math.abs(s.strike - spot) < Math.abs(strikes[bi].strike - spot) ? i : bi, 0)
                        ...
                        const nearIvs = nearAtm
                            .map((s) => Math.max(s.callIV ?? 0, s.putIV ?? 0))
                            .filter((v) => v > 0)
                            .sort((a, b) => a - b);
                        let ivPercentile;
                        if (nearIvs.length >= 3) {
            ...
            try {
                ...
                if (data?.data) {
                    ...
                    const sectors = data.data
                        .filter((idx) => sectorIndices.includes(idx.index))
                        .map((idx) => ({
    ...
    async getEarningsCalendar() {
        return this.cached('intel:earnings-calendar', async () => {
            ...
            try {
                ...
                if (data && Array.isArray(data)) {
                    return data
                        .filter((a) => a.desc?.toLowerCase().includes('financial result') || a.desc?.toLowerCase().includes('board meeting'))
                        .slice(0, 25)
                        .map((a) => ({
                        symbol: a.symbol ?? '',
        ...
        return this.cached('intel:smart-money', async () => {
            ...
            for (const deal of blockDeals) {
                ...
                if (deal.buyOrSell === 'Buy' || deal.buyOrSell === 'B') {
                    ...
                    if (deal.clientName)
                        entry.buyers.push(deal.clientName);
                }
                else {
                    entry.totalSellValue += value;
                    if (deal.clientName)
                        entry.sellers.push(deal.clientName);
                }
    ...
    async getInsiderTransactions() {
        return this.cached('intel:insider-txns', async () => {
            try {
                ...
                if (data?.data && Array.isArray(data.data)) {
                    return data.data.slice(0, 30).map((t) => ({
                        symbol: t.symbol ?? '',
    ...
    async getInsiderSelling(symbol) {
        return this.cached(`intel:insider-selling:${symbol}`, async () => {
            const allTxns = await this.getInsiderTransactions();
            const symbolTxns = Array.isArray(allTxns) ? allTxns.filter((t) => t.symbol === symbol) : [];
            const sellingTxns = symbolTxns.filter((t) => t.transactionType?.toLowerCase().includes('sell') || t.transactionType === 'Disposal');
            return {
                symbol,
                transactions: sellingTxns.slice(0, 10),
                hasRecentSelling: sellingTxns.length > 0,
                totalSellValue: sellingTxns.reduce((s, t) => s + (t.value ?? 0), 0),
            };
        });
    }
    // ── Options Chain ──
    async getBreezeCredentials() {
        try {
    ...
    breezeRequest(path, headers, body) {
    ...
    async fetchOptionsChainFromBreeze(symbol) {
    ...
    async fetchOptionsChain(symbol) {
        ...
        try {
            ...
            console.warn(`[Intelligence] ${symbol} Bridge chain error: ${err?.message}`);
        }
    ...
    jsBlackScholes(s, k, t, r, sigma, isCall) {
        if (t <= 0) {
