...
export class AnalyticsService {
    ...
    async getTradeStats(userId, fromDate, toDate) {
        ...
        if (fromDate)
            where.exitTime = { ...(where.exitTime || {}), gte: new Date(fromDate) };
        if (toDate)
            where.exitTime = { ...(where.exitTime || {}), lte: new Date(toDate) };
    ...
    async getSymbolBreakdown(userId) {
        const portfolios = await this.prisma.portfolio.findMany({
            where: { userId }, select: { id: true },
        });
        if (portfolios.length === 0)
    ...
    async getStrategyBreakdown(userId) {
        const portfolios = await this.prisma.portfolio.findMany({
            where: { userId }, select: { id: true },
        });
        if (portfolios.length === 0)
    ...
    async exportTradesCSV(userId) {
        ...
        const rows = trades.map(t => `${istDateStr(t.exitTime)},${t.symbol},${t.side},${t.entryPrice},${t.exitPrice},${t.qty},${t.grossPnl},${t.totalCosts},${t.netPnl},${t.strategyTag || 'Manual'},${t.holdDuration || ''}`);
