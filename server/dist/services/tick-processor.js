...
export function ingestTick(symbol, tick) {
    const now = Date.now();
    ...
    if (last && minuteKey === lastMinuteKey) {
        ...
        if (buf.candles.length > MAX_BUFFER_SIZE)
            buf.candles.shift();
    }
...
async function processBuffer(buf) {
    if (!isEngineAvailable())
        return;
    try {
        const result = await engineSignals({ candles: buf.candles });
        if (!result)
            return;
        ...
        if (ema9 !== null)
            indicators.ema9 = round2(ema9);
        if (ema21 !== null)
            indicators.ema21 = round2(ema21);
        if (rsi !== null)
            indicators.rsi = round2(rsi);
        if (macdH !== null)
            indicators.macd_histogram = round2(macdH);
        let signal = 'NEUTRAL';
        let confidence = 0.5;
        if (ema9 !== null && ema21 !== null && rsi !== null) {
            if (ema9 > ema21 && rsi < 70) {
                signal = 'BULLISH';
                confidence = 0.5 + Math.min((ema9 - ema21) / ema21 * 10, 0.3);
            }
            else if (ema9 < ema21 && rsi > 30) {
                signal = 'BEARISH';
                confidence = 0.5 + Math.min((ema21 - ema9) / ema21 * 10, 0.3);
            }
        }
...
export async function processRegimeDetection(candles) {
    if (!isEngineAvailable() || candles.length < 50)
        return;
    try {
        const result = await engineFeatureStore({ command: 'detect_regime', candles });
...
export async function processAnomalyDetection(candles, symbol) {
    if (!isEngineAvailable() || candles.length < 30)
        return;
    try {
        const result = await engineFeatureStore({ command: 'detect_anomalies', candles });
...
export function getActiveBuffers() {
    return [...tickBuffers.keys()];
}
function round2(v) {
    return Math.round(v * 100) / 100;
}
//# sourceMappingURL=tick-processor.js.map