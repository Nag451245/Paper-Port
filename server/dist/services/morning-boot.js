...
export class MorningBoot {
    ...
    async runMorningBoot() {
        if (this.running)
            return { usersProcessed: 0, strategiesActivated: 0 };
        this.running = true;
        try {
            ...
            for (const user of users) {
                try {
                    ...
                    console.error(`[MorningBoot] Error for user ${user.id}:`, err.message);
                }
            }
    ...
    async activateOptimizedParams(userId, activeParams) {
        for (const param of activeParams) {
            if (param.source !== 'backtest_optimized')
                continue;
            try {
    ...
    async adjustSignalThresholds(userId, since) {
    ...
    async adjustBotStrategies(userId, insight) {
        if (!insight)
            return 0;
        ...
        try {
            ...
            preMarketSummary = `Pre-market: VIX=${vix.value}, Indices=${JSON.stringify(indices.slice(0, 3))}`;
        }
        catch {
        ...
        let topLosers = [];
        try {
            topWinners = JSON.parse(insight.topWinningStrategies);
        }
        catch { /* empty */ }
        try {
            topLosers = JSON.parse(insight.topLosingStrategies);
        }
        catch { /* empty */ }
        let gptRecommendation;
        try {
            gptRecommendation = await chatCompletionJSON({
                messages: [
                    {
                        role: 'system',
...
Currently active bots: ${bots.map(b => `${b.name} (${b.assignedStrategy || 'none'})`).join(', ')}`,
    ...
    async applyRegimeAdaptation(userId, insight) {
    ...
    async configureRegimeRiskLimits(userId, insight) {
        const regime = insight?.marketRegime ?? 'UNKNOWN';
    ...
    async precomputeWatchlistSignals(userId) {
        try {
    ...
    async loadRegimeHistory(userId) {
        try {
    ...
    async logBootSummary(userId, insight, strategiesActivated) {
        const bots = await this.prisma.tradingBot.findMany({
            where: { userId, isActive: true },
            take: 1,
        });
...
//# sourceMappingURL=morning-boot.js.map